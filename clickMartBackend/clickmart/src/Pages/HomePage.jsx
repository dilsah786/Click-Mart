...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const token = 'your_bearer_token'; // You will need to replace this with the actual token
    ...
    <div className={`${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'} min-h-screen`}>
      ...
      <div className="max-w-screen-xl mx-auto py-8 px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          <Link to="/category/electronics" className="bg-blue-100 p-4 rounded-lg text-center">
            Electronics
          </Link>
          <Link to="/category/fashion" className="bg-blue-100 p-4 rounded-lg text-center">
            Fashion
          </Link>
          <Link to="/category/home-appliances" className="bg-blue-100 p-4 rounded-lg text-center">
            Home Appliances
          </Link>
          <Link to="/category/books" className="bg-blue-100 p-4 rounded-lg text-center">
            Books
          </Link>
        </div>
      </div>
