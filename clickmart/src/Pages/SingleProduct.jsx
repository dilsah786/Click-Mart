...
const SingleProduct = () => {
  ...
  useEffect(() => {
   ...
   const addItemToCart = async (id) =>{
      try {
        ...
        const response = await fetch(`http://localhost:8080/products/cart/add_item_in_cart`, {
          method:"POST",
          headers: {
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-screen">Loading...</div>; // Show loader when fetching data
  }
      ...
      {product ? (
        <div className="flex flex-col lg:flex-row gap-10 items-center">
          ...
          <div className="flex-1 space-y-4">
            ...
            {/* Add to Cart Button */}
            <button onClick={()=>addItemToCart(id)} key={id} className="bg-red-600 text-white px-6 py-2 rounded-md font-medium hover:bg-red-700 transition duration-300">
              Add to Cart
            </button>
          </div>
        </div>
